...
(function(){ if(typeof window==='undefined') return;
    ...
    function mapRow(row,idx,sheet){ if(!row||!row[0]||!row[1]) return null; const id=row[0], name=row[1], rowNum=idx+2; const comment=(row[2]||''); return { id, name, img:`${id}.jpg`, row:rowNum, dataRow:rowNum, sheet, raw:row, comment }; }
    ...
    function updateParticipantComment(rowNumber,value){ const mainSheet=SHEET_CONFIG.mainSheet; const list=bySheet.get(mainSheet); if(!list) return; const idx=list.findIndex(p=>p.row===rowNumber); if(idx>=0){ list[idx]={ ...list[idx], comment:value }; notify(); } }
    function getAllParticipants(){ const res=[]; bySheet.forEach(l=>res.push(...l)); return res; }
    function getParticipantsForSection(section){ const range=(section==='One'?SECTION_RANGES.section1:section==='Two'?SECTION_RANGES.section2:SECTION_RANGES.section3)||[1,1000]; const sheet=SHEET_CONFIG.mainSheet; const list=bySheet.get(sheet)||[]; const [s,e]=range; return list.filter(p=>p.row>=s&&p.row<=e); }
    function subscribe(cb){ subscribers.add(cb); return ()=>subscribers.delete(cb); }
    // --- Патч через AppEvents ---
    if (window.AppEvents && typeof AppEvents.on === 'function') {
        AppEvents.on('cellChanged', ({ sheet, row, column, value }) => {
            if (column === 'C') {
                updateParticipantComment(row, value);
            }
        });
    }
